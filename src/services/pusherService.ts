...
class PusherService {
    ...
    try {
      ...
      if (this.pusher && this.pusher.connection) {
        this.pusher.connection.bind('state_change', (states: any) => {
          ...
          if (states.current === 'disconnected' && !this.isIntentionallyClosed) {
        ...
        this.pusher.connection.bind('error', (error: any) => {
          ...
          if (error?.error?.code === 1006 && !this.isIntentionallyClosed) {
            console.warn('[PusherService] Connection abort detected, will attempt reconnection');
            this._handleConnectionLoss();
          }
        });
  ...
  private _handleConnectionLoss() {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(`[PusherService] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
