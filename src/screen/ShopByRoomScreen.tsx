...
const ROOM_ICONS: Record<number, keyof typeof Ionicons.glyphMap> = {
  1: 'bed-outline',
...
interface ShopByRoomScreenProps {
  token?: string | null;
  roomId?: number;
  user?: any;
  cartCount?: number;
...
export default function ShopByRoomScreen({
  token,
  roomId = 1,
  user,
  cartCount = 0,
  ...
  const selectedRoom = useMemo(
    () => ROOMS.find(r => r.room_id === selectedRoomId) || ROOMS[0],
    [selectedRoomId]
  );
  ...
  const fetchProducts = useCallback(async (page: number = 1) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_CONFIG.BASE_URL}/products?room_type=${selectedRoomId}&status=1&page=${page}&per_page=${perPage}`,
        { headers }
      );
      ...
      // Extract total from meta object
