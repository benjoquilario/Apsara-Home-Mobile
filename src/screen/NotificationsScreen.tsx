...
export default function NotificationsScreen({
  ...
  // Virtualized row renderer for the notifications FlatList. Defined here (after
  // all the helpers it closes over) so it can be passed to FlatList instead of
  // mapping the whole array inside a ScrollView — only on-screen rows mount, which
  // matters because the list is unbounded (users accumulate 100s over time).
    ...
    return (
      <View
        style={[
          ...
          {
            backgroundColor: isUnread ? colors.unreadBg : colors.containerBg,
            borderColor: isUnread ? Colors.sky + "55" : colors.border,
          },
        ]}
      >
        <TouchableOpacity
        ...
        >
          ...
          <View style={styles.notificationContent}>
            <View style={styles.notificationHeaderRow}>
              ...
              <Text
                style={[styles.notificationTime, { color: colors.textSec }]}
              >
            ...
            {/* Meta footer: amount · qty · payment + view-order affordance */}
            {(item.amount > 0 ||
              item.quantity > 0 ||
              item.payment_method) && (
            ...
            {(item.updates || []).length > 0 && (
              ...
              >
                <Text
                  style={[styles.updatesToggleText, { color: Colors.sky }]}
                >
                ...
                (update: any, updateIndex: number) => (
                  <View
                    ...
                    style={[
                      ...
                      updateIndex === (item.updates || []).length - 1 &&
                  ...
                  >
                    ...
                    <View style={styles.updateContent}>
                      <View style={styles.updateHeaderRow}>
                        ...
                        >
                          {formatDate(
                            update.event_date || update.created_at
                          )}
                        </Text>
                      </View>
                      <Text
  ...
  const listData = getFilteredNotifications()
  // Bump FlatList's extraData when an item expands or its updates finish loading
  // (and on theme change) so virtualized rows re-render even though `listData`
  // itself didn't change identity.
  const listExtraData = `${expandedNotificationId}|${Object.keys(loadingUpdates)
    .filter((k) => loadingUpdates[Number(k)])
    .join(",")}|${isDarkMode}`
